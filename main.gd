...
func _ready() -> void:
	...
	for i in 4:
		...
		rl.position =  Vector3(cardsize.x*2,radius,0) + Vector3(i*cardsize.x+i, 0, 0)
		add_child(rl)
	$Arrow3D.set_color(random_color()).set_size(5,0.2,0.6)
	$Arrow3D.position = Vector3(cardsize.x*2,radius,0) + Vector3(-5,0,radius)
